...
public class MainActivity extends Activity {
    ...
    private Button
            ...
            sqrt, square, factorial, del,        //根号  平方  阶乘   退格
            left, right, dot,drg,          //（     ）  .  退出     角度弧度控制键
            mc,c,bin,oct,hex;                                // mem清屏键    input清屏键
    ...
    public boolean timer_lock = true;
     ...
     */
    //命令缓存，用于检测输入合法性
    String[] Timercommand = new String[500];
    //Timercommand的指针
    int timer_i
            = 0;
    private OnClickListener actionPerformed = new OnClickListener() {
        public void onClick(View v) {
            ...
            if (command.compareTo("二进制")==0){
                ...
                input.setText( Integer.toBinaryString( n ) );
            }
            if (command.compareTo("八进制")==0){
                int n = Integer.parseInt( str );
                input.setText( Integer.toOctalString( n ) );
            }
            if (command.compareTo("十六进制")==0){
                int n = Integer.parseInt( str );
                input.setText( Integer.toHexString( n ) );
            }
            //检测输入是否合法
            ...
            if(timer_i
                    > 0)
                TimerChecker(Timercommand[timer_i
                        -1] , command);
            else if(timer_i
                    == 0) {
                TimerChecker("#" , command);
            }
            if("0123456789.()sincostanlnlogn!+-×÷√^".indexOf(command) != -1 && timer_lock) {
                Timercommand[timer_i
                        ] = command;
                timer_i
                        ++;
            ...
            if("0123456789.()sincostanlnlogn!+-×÷√^".indexOf(command) != -1
            ...
            } else if(command.compareTo("DRG") == 0 && timer_lock) {
                if(drg_flag == true) {
                    drg_flag = false;
                    _drg.setText("   RAD");
                } else {
                ...
                mem.setText("0");
            }else if(command.compareTo("=") == 0 && timer_lock && right(str) && equals_flag) {
                timer_i
                        = 0;
                //表明不可以继续输入
                timer_lock = false;
    ...
    /*
     * 判断str是否是合法的，返回值为true、false
     * 含0123456789.()sincostanlnlogn!+-×÷√^的是合法的str，返回true
     * 含除0123456789.()sincostanlnlogn!+-×÷√^以外的字符的str为非法的，返回false
     */
    ...
    /*
     * 检测函数，返回值为3、2、1  表示应当一次删除几个？  Three+Two+One = SBW
    ...
    /*
     * 检测函数，对str进行前后语法检测
     * 为Timer的提示方式提供依据，与TimerShow()配合使用
     *  编号 字符    其后可以跟随的合法字符
     *   1  （                 数字|（|-|.|函数
     *   2   ）                算符|）|√ ^
     *   3  .      数字|算符|）|√ ^
     ...
     * 数字第一位可以为0
     */
    private void TimerChecker(String timer1,String timer2) {
        //Timercode1表示错误类型，Timercode2表示名词解释类型
        int Timercode1 = 0 , Timercode2 = 0;
        //表示命令类型
        int timertype1 = 0 , timertype2 = 0;
        //括号数
        int bracket = 0;
        //“+-x÷√^”不能作为第一位
        if(timer1.compareTo("#") == 0 && (timer2.compareTo("÷") == 0 ||
                timer2.compareTo("×") == 0 || timer2.compareTo("+") == 0 ||
                timer2.compareTo(")") == 0 || timer2.compareTo("√") == 0 ||
                timer2.compareTo("^") == 0)) {
            Timercode1 = -1;
        }
        //定义存储字符串中最后一位的类型
        else if(timer1.compareTo("#") != 0) {
            if(timer1.compareTo("(") == 0) {
                timertype1 = 1;
            } else if(timer1.compareTo(")") == 0) {
                timertype1 = 2;
            } else if(timer1.compareTo(".") == 0) {
                timertype1 = 3;
            } else if("0123456789".indexOf(timer1) != -1) {
                timertype1 = 4;
            } else if("+-×÷".indexOf(timer1) != -1) {
                timertype1 = 5;
            } else if("√^".indexOf(timer1) != -1) {
                timertype1 = 6;
            } else if("sincostanloglnn!".indexOf(timer1) != -1) {
                timertype1 = 7;
            }
            //定义欲输入的按键类型
            if(timer2.compareTo("(") == 0) {
                timertype2 = 1;
            } else if(timer2.compareTo(")") == 0) {
                timertype2 = 2;
            } else if(timer2.compareTo(".") == 0) {
                timertype2 = 3;
            } else if("0123456789".indexOf(timer2) != -1) {
                timertype2 = 4;
            } else if("+-×÷".indexOf(timer2) != -1) {
                timertype2 = 5;
            } else if("√^".indexOf(timer2) != -1) {
                timertype2 = 6;
            } else if("sincostanloglnn!".indexOf(timer2) != -1) {
                timertype2 = 7;
            }
            ...
            switch(timertype1) {
                case 1:
                    ...
                    if(timertype2 == 2 || (timertype2 == 5 && timer2.compareTo("-") != 0) ||
                            timertype2 == 6)
                        Timercode1 = 1;
                    break;
                case 2:
                    //右括号后面接左括号，数字，“+-x÷sin^...”
                    if(timertype2 == 1 || timertype2 == 3 || timertype2 == 4 || timertype2 == 7)
                        Timercode1 = 2;
                    break;
                case 3:
                    //“.”后面接左括号或者“sincos...”
                    if(timertype2 == 1 || timertype2 == 7)
                        Timercode1 = 3;
                    //连续输入两个“.”
                    if(timertype2 == 3)
                        Timercode1 = 8;
                ...
                case 5:
                    //“+-x÷”后面直接接右括号，“+-x÷√^”
                    if(timertype2 == 2 || timertype2 == 5 || timertype2 == 6)
                        Timercode1 = 5;
                    break;
                case 6:
                    //“√^”后面直接接右括号，“+-x÷√^”以及“sincos...”
                    if(timertype2 == 2 || timertype2 == 5 || timertype2 == 6 || timertype2 == 7)
                        Timercode1 = 6;
                    break;
                case 7:
        ...
        //检测小数点的重复性，Timerconde1=0,表明满足前面的规则
        if(Timercode1 == 0 && timer2.compareTo(".") == 0) {
            int timer_point = 0;
            for(int i = 0;i < timer_i
                    ;i++) {
                //若之前出现一个小数点点，则小数点计数加1
                if(Timercommand[i].compareTo(".") == 0) {
                    timer_point++;
                }
                //若出现以下几个运算符之一，小数点计数清零
                if(Timercommand[i].compareTo("sin") == 0 || Timercommand[i].compareTo("cos") == 0 ||
                        Timercommand[i].compareTo("tan") == 0 || Timercommand[i].compareTo("log") == 0 ||
                        Timercommand[i].compareTo("ln") == 0 || Timercommand[i].compareTo("n!") == 0 ||
                        Timercommand[i].compareTo("√") == 0 || Timercommand[i].compareTo("^") == 0 ||
                        Timercommand[i].compareTo("÷") == 0 || Timercommand[i].compareTo("×") == 0 ||
                        Timercommand[i].compareTo("-") == 0 || Timercommand[i].compareTo("+") == 0 ||
                        Timercommand[i].compareTo("(") == 0 || Timercommand[i].compareTo(")") == 0 ) {
                    timer_point = 0;
        ...
        if(Timercode1 == 0 && timer2.compareTo("=") == 0) {
            ...
            if(timer_bracket > 0) {
                Timercode1 = 9;
                bracket = timer_bracket;
            } else if(timer_bracket == 0) {
                //若前一个字符是以下之一，表明=号不合法
                if("√^sincostanloglnn!".indexOf(timer1) != -1) {
                    Timercode1 = 6;
                }
                //若前一个字符是以下之一，表明=号不合法
                if("+-×÷".indexOf(timer1) != -1) {
        ...
        //若命令式以下之一，则显示相应的帮助信息
        if(timer2.compareTo("MC") == 0) Timercode2 = 1;
        if(timer2.compareTo("C") == 0) Timercode2 = 2;
        if(timer2.compareTo("DRG") == 0) Timercode2 = 3;
        if(timer2.compareTo("Del") == 0) Timercode2 = 4;
        if(timer2.compareTo("sin") == 0) Timercode2 = 5;
        if(timer2.compareTo("cos") == 0) Timercode2 = 6;
        if(timer2.compareTo("tan") == 0) Timercode2 = 7;
        if(timer2.compareTo("log") ==0) Timercode2 = 8;
        if(timer2.compareTo("ln") == 0) Timercode2 = 9;
        if(timer2.compareTo("n!") == 0) Timercode2 = 10;
        if(timer2.compareTo("√") == 0) Timercode2 = 11;
        if(timer2.compareTo("^") == 0) Timercode2 = 12;
        //显示帮助和错误信息
    }

    /*
     * 反馈Timer信息，加强人机交互，与TimerChecker()配合使用
     */
    public class calc {
        public calc(){
